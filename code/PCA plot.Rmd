...
p1 <- ggplot(pcaData, aes(x = PC1, y =PC2, shape=Group, color = Stage))+
  ...
  ggtitle("PCA plot - Unnomalized data") + theme(plot.title = element_text(hjust = 0.5)) + theme(
...
pca_se(se=se, assay = 1, group=cd$Group, pcaData$Stage)
...
colgroup <- c(cols[3],cols[7])
colgroup <- c(Control = cols[3], MSUS = cols[7])
```
